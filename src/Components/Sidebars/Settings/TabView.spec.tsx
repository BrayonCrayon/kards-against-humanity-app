import TabView, { INavContainer, Tab } from "./TabView";
...
const renderComponent = ({ tabs } : INavContainer) => {
    return render(<TabView tabs={tabs}/>)
}
describe("TabView", () => {

    it('will render out jsx element', () => {
        const tabs: Tab[] = [{ key: 'test-nav', element: <TestElement /> }];
    ...
    it('will render out jsx elements conditionally', () => {
        const tabs: Tab[] = [
            { key: 'tab-key-1', element: () => { return <div data-testid="tab-1">tab 1</div> } },
            { key: 'tab-key-2', element: () => { return <div data-testid="tab-2">tab 2</div> } }
        ];
